...
curl -i -H "Accept: application/json" -H "X-AN-APP-NAME: moodle" -H "X-AN-APP-KEY: $APPKEY" -X POST -d "alert=$ALERT&device=$DEVICE&token=$TOKEN" http://localhost:8801/notification/
